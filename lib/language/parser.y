...
%token <string> TIDENTIFIER TINTEGER TYUINT TYUINT1 TYUINT4 TYUINT6 TYUINT8 TYUINT12 TYUINT16 TYUINT32 TYUINT64 TYUINT128
%token <token>  TFOR TRETURN TFROM TTO TVIEW TTOUINT
%token <token>  TCIPHER TSPN TBFN TBSIZE TRNUM TSSIZE TNIBSIZE
%token <token>  TSBOX TPBOX TPBOXM
%token <token>  TFFM TRFN TSCHDFN TSFN TFN
%token <token>  TPLUS TMINUS TDIVIDE TFFTIMES TCOMMA TASSIGN TSEMICOLON
%token <token>  TSHIFTL TSHIFTR TRSHIFTL TRSHIFTR TLPAREN TRPAREN TLBRACKET TRBRACKET TLBRACE TRBRACE TLABRACKET TRABRACKET
%token <token>  TNOT TMOD TAND TOR TXOR
...
// 这里不能递归实现了，只能用一维数组（sbox，pbox）和二维数组（pboxm，ffm）两种,分别由array_typename1 和 array_typename2实现
array_typename1
...
//	| array_typename1 ident TASSIGN expr {
//	    $$ = new NArrayInitialization(
//	        std::make_shared<NVariableDeclaration>(std::shared_ptr<NIdentifier>($1), std::shared_ptr<NIdentifier>($2), nullptr),
//	        std::shared_ptr<NExpression>($4));
//	}
	;
...
//	: ident TLBRACKET expr TCOLON expr TRBRACKET {
		//$$ = new NArrayRange(std::shared_ptr<NIdentifier>($1), std::shared_ptr<NInteger>($3), std::shared_ptr<NInteger>($5));
//	}
//	;
