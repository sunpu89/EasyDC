...
ProcedureH::ProcedureH(string name, const vector<vector<ThreeAddressNodePtr>> &parameters, const vector<ThreeAddressNodePtr> &block,
                       const vector<ThreeAddressNodePtr> &returns, const map<string, ProcedureHPtr>& nameToProc) : name(name), parameters(parameters), block(block),
                                                                                                                   returns(returns), nameToProc(nameToProc), isSimple(true) {}
...
bool ProcedureH::isSimple1() const {
    return isSimple;
}

void ProcedureH::setIsSimple(bool isSimple) {
    ProcedureH::isSimple = isSimple;
}
...
//vector<ThreeAddressNodePtr> ProcedureH::instantitateProcedureCall(ProcedureHPtr proc, ThreeAddressNodePtr callInstruction,map<string, ThreeAddressNodePtr>& saved, map<ThreeAddressNodePtr, ThreeAddressNodePtr>& savedForFunction,
//                                                                      map<ThreeAddressNodePtr, ThreeAddressNodePtr>& formalToActual, string path, set<ThreeAddressNodePtr>& backup ){
//    // 这里还是需要解决一下newBlock中的问题，这里的newBlock不是单纯的直接获取block就行了
//    // 如果是composite 函数，那么还需要获取到inline后的block，此时inline后的block和直接获取到的block应该在参数上相同
