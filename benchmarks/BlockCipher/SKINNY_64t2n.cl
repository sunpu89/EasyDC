...
r_fn uint1[64] round_function(uint8 r, uint1[128] key, uint1[64] input) {
    uint1[64] s_out;
    for (i from 0 to 15) {
    ...
    #uint6 c = rc[r];
    #uint4 c0 = touint(c[0], c[1], c[2], c[3]);
    #uint4 c1 = touint(0, 0, c[5], c[4]);
    #uint4 c2 = 2;
    #for (i from 0 to 3) {
    #    s_out[i] = s_out[i] ^ c0[3-i];
    #    s_out[i + 16] = s_out[i + 16] ^ c0[3-i];
    #    s_out[i + 32] = s_out[i + 32] ^ c0[3-i];
    #}

    uint1[64] t_out = s_out;
    for (i from 0 to 31) {
        t_out[i] = s_out[i] ^ key[i] ^ key[i + 64];
    }
...
fn uint1[64] enc(uint1[4608] key, uint1[64] r_plaintext){
    for (i from 1 to 36) {
        r_plaintext = round_function(i, View(key, (i - 1) * 128, i * 128 - 1), r_plaintext);
    }
    return r_plaintext;
}
